...
class ProfileRepository:
    ...
    async def ensure_user_profile_by_email(self, email: str) -> UserProfile:
        try:
            result = await self.db.execute(
            ...
            profile = result.scalar_one_or_none()
            if not profile:
                auth_user = await self.get_auth_user_by_email(email)
            ...
            await self.db.rollback()
            raise Exception(f"Failed to ensure user profile by email: {str(e)}")
